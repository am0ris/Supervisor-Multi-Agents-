...
# ---- نموذج graph وهمي بسيط قبل ما نستورد app.py ----
import graph.builder as builder_module


class _FakeCompiledGraph:
    async def _astream(self, initial_state, config, stream_mode):
        steps = [
    ...
    async def astream(self, initial_state, config, stream_mode):
        async for x in self._astream(initial_state, config, stream_mode):
            yield x
...
print("Exception after click:", at.exception)
assert not at.exception, f"حصل استثناء بعد الضغط على الزرار: {at.exception}"

markdowns = [m.value for m in at.markdown]
full_text = "\n".join(markdowns)
print("Contains final summary:", "ملخص تجريبي نهائي" in full_text)
assert "ملخص تجريبي نهائي" in full_text

print("\n✅ تدفق الضغط على الزرار كامل شغال: من الإدخال لحد عرض الملخص النهائي")
