"""Builds the full LangGraph: supervisor + all agents + correct edges."""
...
def make_supervisor_node(supervisor_llm):
    async def supervisor_node(state: ResearchState):
        if state["step_count"] >= config.MAX_TOTAL_STEPS:
            return {
                "next_agent": "finish",
                "supervisor_reason": "Reached the overall maximum step limit.",
        ...
        try:
            ...
            raw_reason = f"{raw_reason} (LLM call failed: {e})"
...
async def build_graph():
    ...
    search_jobs_tool = tools.get("search_jobs")  # optional - only used for job_search queries

    builder = StateGraph(ResearchState)

    builder.add_node("supervisor", make_supervisor_node(supervisor_llm))
    builder.add_node("research", make_research_agent(search_tool, search_jobs_tool))
    builder.add_node("extraction", make_extraction_agent(fetch_tool, llm))
