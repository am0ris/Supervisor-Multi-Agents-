...
with st.sidebar:
    ...
    st.markdown("**Agent Limits**")
    for agent, limit in config.AGENT_LIMITS.items():
        st.markdown(f"- {AGENT_ICONS.get(agent, '•')} `{agent}`: **{limit}** Try")

    st.divider()
    if config.LANGSMITH_ENABLED:
        st.success(f"LangSmith working ✅\nProject: `{config.LANGSMITH_PROJECT}`")
    else:
        st.warning("")
...
if run_clicked:
    ...
    try:
        with st.spinner("Working on agents and MCP server"):
            graph = get_graph()
    ...
    progress_container = st.status("🏃 Project Working...", expanded=True)
    step_log = []
    ...
    def on_update(node, output):
        ...
        if node == "supervisor":
            ...
            line = f"{icon} **{node}** Working (Try num {calls})"
        step_log.append(line)
        progress_container.write(line)

    try:
        run_graph_sync(graph, initial_state, thread_config, on_update)
        progress_container.update(label=" Complete", state="complete", expanded=False)
    except Exception as e:
